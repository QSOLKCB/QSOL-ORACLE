...
from nexus_membrane_common import MembraneError, build_envelope, validate_envelope, is_sha256

VIEW_PROTOCOL = "QSOL-ORACLE-STENOGRAPHER-VIEW/1"
STENO_BOUNDARY = {
    "target_surface": "Courtroom Stenographer", "native_schema": "nexus-stenographer/1",
    "native_record": False, "native_record_scope": "ai_actions_only",
    "external_view_scope": "oracle_evidence", "append_to_native_stenographer_ledger": False,
    "read_only": True,
}
...
def build_view(event: dict[str, Any], *, request_id: str) -> dict[str, Any]:
    ...
    view = {
        "protocol": VIEW_PROTOCOL, "view_type": "oracle_witness_event", "event_hash": event["event_hash"],
        "sequence": event.get("sequence"), "subject": event.get("subject"), "event_type": event.get("event_type"),
        "observed_at": event.get("observed_at"), "source": copy.deepcopy(event.get("source")),
        "provenance_kind": event.get("provenance_kind"), "evidence": copy.deepcopy(event.get("evidence")),
        "authority": "presentation-only", "truth_claim": False, "hidden_reasoning_included": False,
        "nexus_stenographer": copy.deepcopy(STENO_BOUNDARY),
    }
    view["view_sha256"] = sha256_value(view)
    validate_view_payload(view)
    return build_envelope(direction="oracle-to-nexus", kind="stenographer_event_view", request_id=request_id, payload=view)


def validate_view_payload(view: dict[str, Any]) -> None:
    required = {"protocol", "view_type", "event_hash", "sequence", "subject", "event_type", "observed_at", "source", "provenance_kind", "evidence", "authority", "truth_claim", "hidden_reasoning_included", "nexus_stenographer", "view_sha256"}
    ...
    payload = copy.deepcopy(view); supplied = payload.pop("view_sha256")
